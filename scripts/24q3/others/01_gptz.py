...
def test_encode_and_decode_is_lossless():
    ...
    print(f"Code: {to_binary(code)} ({len(code)} bytes)")  # type: ignore
    decoded_string = coder.decode(code, num_padded_bits=num_padded_bits)  # type: ignore
    assert decoded_string == string


if __name__ == "__main__":
    test_encode()
    test_encode_and_decode_is_lossless()
